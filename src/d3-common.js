...
const isFunction = d3.isFunction = _ => typeof _ == 'function';
const isArray = d3.isArray = Array.isArray;
const isNumber = d3.isNumber = _ => _ != null && _.constructor == Number;
const isString = d3.isString = _ => _ != null && _.toString === String.prototype.toString;
const isDOM = d3.isDOM = el => el instanceof Element;
const toArray = d3.toArray = a => (a!=null?Array.isArray(a)? a:[a]:[]);
const trim = d3.trim = s => s.replace(/^\s+/,"").replace(/\s+$/,"");
const strip = d3.strip = s => (""+s).replace(/\s+/g," ").replace(/(^\s+|\s+$)/g,"");
const tostr = s => s? isDOM(s)? s.outerHTML:isFunction(s)? s(): isArray(s)? s.map(trim).filter(t=>t).join("<br/>"): s: null;
const L2 = d3.L2 = n => ("00"+n).slice(-2);
const L5 = d3.L5 = n => ("00000"+n).slice(-5);
////-----------------------------------------------------------------------------------
...
d3.thenEach = async(a, func) => { const n = a.length;
    for (let i = 0; i < n; i++) await func(a[i],i).catch(console.error);
};
d3.thenEachPref = (()=>{
    const a = []; for (let i = 1; i <=47; i++) a.push(L2(i));
    return f=>d3.thenEach(a,f)
})();
d3.thenMap = async(a, func) => { const n = a.length, q = [];
    for (let i = 0; i < n; i++) q.push(await func(a[i],i).catch(console.error));
    return q;
};
d3.thenMapPref = (()=>{
    const a = []; for (let i = 1; i <=47; i++) a.push(L2(i));
    return f=>d3.thenMap(a,f)
})();
d3.thread = async(a, n, func) => {
    const results = [];
...
d3.unique = a => Array.from(new Set(a).values());
d3.concat = a => { let q = []; a.forEach(t =>q = q.concat(t)); return q; };
d3.slice = (a, n) => { var q = [], len = a.length;
    for (var i = 0; i < len; i += n) q.push(a.slice(i, i + n));
    return q;
};
d3.random = a => {
...
d3.xy2yx = a => { let q = [];
    a.forEach((t,n) => { t.forEach((t,m) =>{ q[m] = q[m]||[]; q[m][n] = t;})});
    return q;
};
